...
export default async function AdminQuotesPage() {
  ...
  const { data: quoteRows, error: quotesError } = await supabase
    .from("quotes")
    .select("id, folio, customer_id, seller_id, machine_name_snapshot, total, status, created_at")
  ...
  const sellerIds = (quoteRows ?? []).map((quote) => quote.seller_id);
  const [{ data: customerRows, error: customersError }, { data: sellerRows, error: sellersError }] =
    await Promise.all([
      customerIds.length
  ...
  if (customersError || sellersError) {
  ...
  const sellerNames = new Map(
    (sellerRows ?? []).map((seller) => [seller.id, seller.full_name])
  );
