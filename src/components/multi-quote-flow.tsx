...
export function MultiQuoteFlow({ machine, addons, variants = [], isExpoAccount = false, onChangeMachine, onViewPending }: {
  ...
  function editItem(item: CartItem) {
    ...
    setQuantities(item.addonQuantities); setQuantityInputs({}); setCoupon(null); setStep("configure"); setError("");
  }
  ...
  return <main className="mx-auto grid max-w-5xl gap-6 px-5 py-8">
    ...
    {step === "configure" ? <section className={panelClass}>
      ...
      {config.addons.length ? <fieldset className="grid gap-3"><legend className="mb-3 font-bold">Add-ons por máquina</legend>{config.addons.map((addon) => { const max=addonQuantityMax(config.machine,addon); const quantityValue=quantities[addon.id] ?? 0; return <label key={addon.id} className="flex items-center justify-between gap-4 rounded-xl border border-border p-4"><span><strong>{addon.name}</strong><span className="block text-sm text-muted">{addon.description}</span><span className="block">{money(grossToNet(addon.unitPrice))}{addon.calculationType === "PER_BASE" ? ` × ${config.machine.numberOfBases} bases × ${quantity} equipos` : " por unidad"}</span></span>{addon.calculationType === "QUANTITY" ? <input aria-label={`Cantidad de ${addon.name} por máquina`} className={`${inputClass} max-w-24`} type="number" min={addon.required ? 1 : 0} max={max??undefined} step={1} value={quantityInputs[addon.id] ?? String(quantityValue)} onChange={(e) => { const raw=e.target.value; setQuantityInputs((current) => ({ ...current, [addon.id]: raw })); if(raw !== ""){ const parsed=Number(raw); if(Number.isFinite(parsed)){ const next=Math.min(max??10000,Math.max(0,Math.trunc(parsed))); setQuantities((current) => updateQuantity(current,addon.id,next)); setQuantityInputs((current) => ({ ...current, [addon.id]: String(next) })); } } }} onBlur={() => { if(quantityInputs[addon.id] === ""){ setQuantities((current) => updateQuantity(current,addon.id,0)); setQuantityInputs((current) => { const next={...current}; delete next[addon.id]; return next; }); } }} /> : <input aria-label={addon.name} type="checkbox" disabled={addon.required} checked={quantityValue > 0} onChange={(e) => setQuantities(updateQuantity(quantities, addon.id, e.target.checked ? 1 : 0))} />}</label>; })}</fieldset> : null}
...
function addonQuantityMax(machine: SellerMachine, addon: SellerAddon) { const n=addon.name.toLowerCase(); if(n.includes("láser en cruz extra")||n.includes("laser en cruz extra")) return 4; if(n.includes("doble tiempo")) return machine.name.toLowerCase().includes("ome")||machine.slug.includes("ome")?2:1; return addon.calculationType === "QUANTITY" ? 10000 : undefined; }
