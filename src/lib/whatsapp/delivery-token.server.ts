...
export async function getOrCreateDeliveryToken(quoteId: string) {
  ...
  if (existing.error) throw new Error("No se pudo preparar el código de WhatsApp.");
  ...
  if (inserted.error || !inserted.data) throw new Error("No se pudo generar el código de WhatsApp.");
