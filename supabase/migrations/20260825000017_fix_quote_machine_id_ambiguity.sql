...
-- quote_variant_configuration returns a column named machine_id. In PL/pgSQL,
-- that output variable conflicted with unqualified machine_variants.machine_id
-- references. Qualify every machine_variants column explicitly.
create or replace function public.quote_variant_configuration(
  p_machine_id uuid,
...
begin
  ...
  select exists (
    ...
    where machine_variant.machine_id = v_configuration.machine_id
  )
    into v_has_variants;
  ...
  if not v_has_variants then
    ...
    select
      ...
      v_configuration.machine_delivery_policy,
      null::uuid,
      null::public.machine_variant_type,
      null::text,
  ...
  select machine_variant.*
    into v_variant
  from public.machine_variants as machine_variant
  where machine_variant.id = p_machine_variant_id
    and machine_variant.machine_id = v_configuration.machine_id
    and machine_variant.active
    and machine_variant.price is not null
    and machine_variant.price > 0;
...
revoke all on function public.quote_variant_configuration(uuid, jsonb, uuid)
from public, anon, authenticated;

-- Keep the current create_quote signature and behavior. These aliases prevent
-- its result columns folio and total from colliding with quote table columns
-- in the idempotency paths.
...
begin
  ...
  for v_attempt in 1..5 loop
    ...
    begin
      insert into public.quotes as created_quote (
        ...
        v_configuration.subtotal,
        v_discount,
        v_configuration.subtotal - v_discount,
        v_coupon_code,
        v_coupon_name,
        v_coupon_type,
        v_coupon_value,
  ...
  return query
  select v_quote_id, v_folio, v_configuration.subtotal - v_discount;
end;
$$;
